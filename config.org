...
** LLM Clients
*** Helpers to fetch all models
#+begin_src elisp :tangle config.el
(require 'json)
(require 'url)
(defvar my--openai-models-cache (make-hash-table :test 'equal)
  "Cache for fetched OpenAI-compatible model lists.")

(defun fetch-openai-compatible-models (url)
  "Fetch models from an OpenAI-compatible API at URL.
Returns a list of model ID strings, or nil on failure.
Results are cached; use `my/refresh-openai-models' to clear."
  (or (gethash url my--openai-models-cache)
      (condition-case err
          (let* ((url-parsed (url-generic-parse-url url))
                 ...
                 (buf (let ((url-request-extra-headers
                        ...
                        (url-retrieve-synchronously (concat url "/models") nil nil 5))))
            (when buf
              (unwind-protect
                  (with-current-buffer buf
                    ...
                    (let* ((json-object-type 'alist)
                           ...
                           (result (mapcar (lambda (model)
                                             (or (alist-get 'id model)
                                                 (alist-get 'name model)))
                                           models)))
                      (puthash url result my--openai-models-cache)
                      result))
                (kill-buffer buf))))
        (error
         (message "Warning: Failed to fetch models from %s: %s" url (error-message-string err))
         nil))))

(defun my/refresh-openai-models ()
  "Clear the model cache and re-fetch all cached endpoints."
  (interactive)
  (let ((urls (hash-table-keys my--openai-models-cache)))
    (clrhash my--openai-models-cache)
    (dolist (url urls)
      (fetch-openai-compatible-models url))
    (message "Refreshed models for %d endpoints" (length urls))))
#+end_src
...
(after! gptel
  ...
  ;; Register backends with static fallback models (full lists loaded lazily)
  ...
  (gptel-make-ollama "Ollama"
    ...
    :stream t
    :models nil)
  ;; Lazy-load full model lists after Emacs is idle
  (run-with-idle-timer 5 nil
    (lambda ()
      (dolist (entry '(("OpenRouter" . "https://openrouter.ai/api/v1")
                       ("OpenAI" . "https://api.openai.com/v1")))
        (when-let* ((models (fetch-openai-compatible-models (cdr entry)))
                    (backend (alist-get (car entry) gptel--known-backends
                                        nil nil #'equal)))
          (setf (gptel-backend-models backend) models)))
      (when-let* ((models (get-ollama-models))
                  (backend (alist-get "Ollama" gptel--known-backends
                                      nil nil #'equal)))
        (setf (gptel-backend-models backend) models))
      (message "gptel: model lists refreshed")))
  )
(map! :leader
      (:prefix ("G" . "gptel")
