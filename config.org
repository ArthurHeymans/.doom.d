...
#+end_src
** org-node
Try org-node alongside org-roam for faster node indexing and no SQLite DB
(better for Syncthing sync). Enables =org-node-roam-accelerator-mode= to
speed up org-roam's backlink buffer while both coexist.
...
(use-package! org-node
  ...
  :config
  (org-node-cache-mode)
  ;; Speed up org-roam's backlink buffer using org-node's faster index
  (org-node-roam-accelerator-mode)
  ;; Use roam-capture for node creation so existing templates still work
